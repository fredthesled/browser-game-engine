// games/survivors-balance/scenes/survivors-shop.js
// Between-wave shop for Survivors (Engine.Balance fork).
//
// Balance-experiment fork of games/survivors/scenes/survivors-shop.js. The only
// change from v3 is that upgrade pricing is geometric via Engine.Balance.cost()
// (ADR-0020) instead of v3's linear basePrice + purchaseCount * priceInc.
// Everything else is identical.
//
// Upgrade prices: geometric, round(basePrice * rate^purchaseCount) via
//   Engine.Balance.cost(n, { base, rate }). See SURVIVORS_COST below.
...
// Depends on: Engine.Scene, Engine.input, Engine.Balance.
//   SurvivorsMatchScene must be defined in build (transition target).
// Used by: SurvivorsMatchScene (on wave completion).

// ----------------------------------------------------------------------------
// Upgrade cost tunable. Geometric scaling via Engine.Balance.cost(n, opts):
//   price(n) = round(basePrice * rate^n)   where n = times already purchased.
// rate is in balance.md's documented 1.07-1.15 band; 1.12 is deliberately
// gentle. First purchase of each upgrade (n=0) is its basePrice, matching v3.
// Because v3's linear priceInc was steep (e.g. +15 on a base of 20), this
// gentle geometric curve is cheaper than v3 past the first buy, keeping
// repeated upgrades affordable across a short run.
// ----------------------------------------------------------------------------
const SURVIVORS_COST = { rate: 1.12 };
...
class SurvivorsShopScene extends Engine.Scene {
  ...
  _upgradePool() {
    ...
    const rate = SURVIVORS_COST.rate;
    const defs = [
      { id:'maxHealth', label:'MAX HEALTH +25',    desc:'Increase max HP and fully heal.',         basePrice:20, apply:(s)=>{ s.maxHealth+=25; s.currentHealth=s.maxHealth; } },
      { id:'speed',     label:'SPEED +30',          desc:'Move faster.',                             basePrice:22, apply:(s)=>{ s.speed+=30; } },
      { id:'fireRate',  label:'FIRE RATE +20%',     desc:'Shoot more frequently.',                   basePrice:28, apply:(s)=>{ s.fireRate=+(s.fireRate*1.2).toFixed(3); } },
      { id:'damage',    label:'DAMAGE +10',          desc:'More damage per projectile.',              basePrice:24, apply:(s)=>{ s.damage+=10; } },
      { id:'range',     label:'RANGE +60',           desc:'Expand shooting range.',                   basePrice:18, apply:(s)=>{ s.range+=60; } },
      { id:'magnet',    label:'COIN MAGNET +80px',   desc:'Auto-attract nearby coins.',               basePrice:30, apply:(s)=>{ s.magnetRange=(s.magnetRange||0)+80; } },
      { id:'multiShot', label:'MULTI-SHOT (+2)',      desc:'Two more projectiles, always symmetric.',  basePrice:45, apply:(s)=>{ s.projectileCount+=2; } },
      { id:'projSize',  label:'PROJECTILE SIZE +2',  desc:'Larger projectile hitbox.',                basePrice:14, apply:(s)=>{ s.projectileSize+=2; } },
    ];
    // Geometric pricing (ADR-0020): cost(n) = basePrice * rate^n, n = times
    // already bought. First buy (n=0) equals basePrice, identical to v3.
    return defs.map(u => ({
      ...u,
      price: Math.round(Engine.Balance.cost(ul[u.id] || 0, { base: u.basePrice, rate })),
    }));
  }
