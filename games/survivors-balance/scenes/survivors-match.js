// games/survivors-balance/scenes/survivors-match.js
// Core gameplay scene for Survivors (Engine.Balance fork).
//
// This is the balance-experiment fork of games/survivors/scenes/survivors-match.js.
// The only behavioural change from v3 is that the three per-wave difficulty
// ramps (enemy HP, enemy speed, spawn cadence) now route through
// Engine.Balance.difficulty() instead of hand-rolled linear formulas (ADR-0020).
// All ramp constants live in the SURVIVORS_RAMP block below so they can be
// retuned in one place. Everything else is identical to v3.
//
// Why logistic instead of linear: v3's ramps grow without bound (wave 30 enemies
// have ~3.9x HP and climbing). A logistic curve tracks v3 closely through the
// mid game, then plateaus, so late waves stay hard but stop running away. This
// is the difficulty-overcorrection countermeasure the balance-check rule targets.
//
...
// ----------------------------------------------------------------------------
// Difficulty ramp tunables. Each is an Engine.Balance.difficulty() opts object
// with t = wave number (this._level, 1-based). All three are logistic:
//   d(level) = L / (1 + e^(-k*(level - t0)))   in [0, L]
// and are applied as shown in the "applied as" note. Sample values are listed
// so the shape is legible without running the curve.
//
//   hp:    enemy HP multiplier   = 1 + d
//          wave  1 -> ~1.11   5 -> ~1.38   9 -> ~1.95   13 -> ~2.52
//          wave 20 -> ~2.86  30 -> ~2.90 (plateau).  v3 was 1.0/1.4/1.8/2.2/2.9/3.9.
//   speed: enemy speed multiplier = 1 + d (low ceiling; speed runaway is unfair)
//          wave  1 -> ~1.03   9 -> ~1.30  20 -> ~1.60  30 -> ~1.60 (plateau).
//   spawn: spawn interval (seconds) = base - d, clamped to floor
//          wave  1 -> ~1.03   5 -> ~0.78   10 -> ~0.42  18+ -> 0.22 (floor).
//          v3 reached the floor by wave ~9; this fork eases that.
// ----------------------------------------------------------------------------
const SURVIVORS_RAMP = {
  hp:    { type: 'logistic', L: 1.9, k: 0.35, t0: 9 },
  speed: { type: 'logistic', L: 0.6, k: 0.35, t0: 9 },
  spawn: { type: 'logistic', L: 1.0, k: 0.32, t0: 6, base: 1.2, floor: 0.22 },
};
...
class SurvivorsMatchScene extends Engine.Scene {
  ...
  // Spawn cadence: base interval minus a logistic pressure term, clamped to a
  // floor. Faster spawns = harder. (v3: max(0.22, 1.2 - (level-1)*0.12).)
  _getSpawnInterval() {
    const R = SURVIVORS_RAMP.spawn;
    const pressure = Engine.Balance.difficulty(this._level, R);
    return Math.max(R.floor, R.base - pressure);
  }
  ...
  _getEnemyConfig(type) {
    // Logistic ramps (ADR-0020): bounded growth instead of v3's unbounded
    // linear 1 + (level-1)*0.10 (HP) and *0.04 (speed).
    const hm = 1 + Engine.Balance.difficulty(this._level, SURVIVORS_RAMP.hp);
    const sm = 1 + Engine.Balance.difficulty(this._level, SURVIVORS_RAMP.speed);
    const base = {
