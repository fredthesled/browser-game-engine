...
| ParallaxBackground | scripts/parallax-background.js | Implemented | Scrolling parallax background renderer. Draws layered images in screen space (independent of host position) filling the canvas. Each layer has speedX/speedY (px/s), repeat (tile horizontally), and optional tileW (tile width override). Auto-scroll mode: layers self-advance; camera-driven mode: call setCameraX(x) each frame and each layer scrolls at speedX/referenceSpeed * x. Attach host before other objects so background renders first. | update, draw |
