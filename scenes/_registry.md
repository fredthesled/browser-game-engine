...
| LibraryMenuScene | games/marginalia/scenes/menu.js | Implemented | Title screen for Marginalia. Floating dust-mote particles, shows unlocked-spell count. Transitions to LibraryDeckSelectScene on click/Enter. | UNLOCK_SPELL_IDS |
| LibraryDeckSelectScene | games/marginalia/scenes/deck-select.js | Implemented | Pre-run spell selection for Marginalia. Shows full pool (base + unlocked); player picks exactly 5, then transitions to LibraryMatchScene. | SPELLS, BASE_SPELL_IDS, UNLOCK_SPELL_IDS, LibraryMatchScene |
| LibraryMatchScene | games/marginalia/scenes/match.js | Implemented | Core combat for Marginalia. 6 sequential encounters; HP carries across. Turn-based: cast 1 spell/turn (Codex adds +1); cooldown tracking; status effects (marked, stunned, chain bonus, attack modifier); armor damage reduction; alt attacks (Grand Index); periodic enemy healing (Borrower). Each victory unlocks one spell; heal 3 HP between encounters. | PauseOverlay, SPELLS, ENCOUNTERS, ENCOUNTER_ORDER, getEncounter, UNLOCK_SPELL_IDS, LibraryMenuScene, LibraryGameOverScene |
| LibraryGameOverScene | games/marginalia/scenes/game-over.js | Implemented | Win/loss screen for Marginalia. Shows outcome, rooms cleared, HP remaining, turns taken. Returns to LibraryMenuScene. | LibraryMenuScene |
