...
**Decision**: Add a publish step to `build.yml` that creates (or updates in place) a GitHub Release tagged `latest-build` after every successful game build. The release holds the built HTML files as release assets, providing permanent public download URLs of the form `releases/download/latest-build/<game>.html`. Uses `softprops/action-gh-release@v2` with `make_latest: true`. If the tag already exists, the action updates it and re-uploads the assets; no duplicate releases accumulate.

**Context**: Built games were previously only accessible as GitHub Actions workflow artifacts, which expire after 90 days and require repo access to download. A permanent, non-expiring URL is needed for sharing games with players who do not have GitHub accounts.

**Alternatives considered**:

1. **GitHub Pages**: Requires a dedicated `gh-pages` branch and Jekyll configuration, or a GitHub Pages site with its own build step. Adds maintenance overhead and ties distribution to a separate branch.
2. **Cloudflare Pages / Netlify**: Third-party services with more configuration. Useful long-term but overkill for the current scale.
3. **Rolling GitHub Release (chosen)**: A single `latest-build` tag is created once and updated on every push. The rolling-tag pattern is idiomatic on GitHub. Zero extra infrastructure; uses the existing `build.yml` job.

**Consequences**:

- The step runs after all game builds complete in the `build` job. It collects `build/*.html` and uploads them to the `latest-build` release.
- The URL pattern `releases/download/latest-build/<game>.html` is permanent and public (if the repo is public) or accessible to repo members (if private).
- `fail_on_unmatched_files: false` ensures the step is a no-op if no `.html` files were built (e.g., a validate-only dispatch).
- The `build.yml` workflow already has `permissions: contents: write` from ADR-0019's commit-back step; no new permission is required.
- If the repo is made public later, all URLs immediately become public with no migration.

## ADR-0023: Registry validation in CI

Date: 2026-06-20

**Decision**: Add a registry validation step to the `validate` job in `build.yml`. For each of `scripts/` and `scenes/`, the step scans root-level `.js` files (at `maxdepth 1`, ignoring subdirectories) and asserts that each filename appears verbatim in its folder's `_registry.md`. If any file is missing, the job prints a list of missing entries and exits non-zero, blocking the `build` job.

**Context**: `_registry.md` files are the cross-session catalog of shared scripts and scenes (ADR-0003). They are hand-maintained and have no automated enforcement. A file committed to `scripts/` without a registry entry is invisible to new Claude sessions bootstrapping from docs, creating silent knowledge drift.

**Alternatives considered**:

1. **Pre-commit hook**: Runs locally but requires hook installation and fails on machines without it. Not reliable across Claude-session push paths.
2. **Dedicated validation workflow**: A separate YAML file would add clutter. The `validate` job in `build.yml` already exists for `node --check`; extending it is cleaner.
3. **`validate` job extension (chosen)**: The existing step in `build.yml` is the natural home. Failures block the downstream `build` job. The `find ... -maxdepth 1 -name '*.js'` pattern excludes game-specific scenes and scripts (which live in subdirectories), targeting only the shared root-level files.

**Consequences**:

- Any new `.js` file added to `scripts/` or `scenes/` root must have a corresponding entry in `_registry.md` before the CI build passes.
- Files in subdirectories (e.g., `games/pong/scenes/play.js`) are unaffected; the check is `maxdepth 1`.
- `.sh` and other non-`.js` files in `scripts/` are not checked. The registry convention targets shared JavaScript modules.
- All 9 root-level `.js` files present at the time of this ADR pass the check. No retroactive registry updates were needed.
- The validation runs before game builds, so a registry gap blocks the entire build job cleanly.

## ADR-0024: Game scaffolding script and workflow

Date: 2026-06-20

**Decision**: Add `scripts/scaffold-game.sh`, a bash script that creates a minimal game skeleton under `games/<name>/`: a `build-manifest.json` wired to `bootstrapGame` and a placeholder `MenuScene` in `games/<name>/scenes/menu.js`. Add `.github/workflows/scaffold.yml`, a `workflow_dispatch` workflow with `name` and `title` inputs that runs the script and commits the created files back via `stefanzweifel/git-auto-commit-action@v5`.

**Context**: Starting a new game required manually creating a directory, writing a manifest, and writing a scene — three steps with a specific schema that Claude must recall or re-derive each session. The scaffolding script captures this exactly once and makes it callable from the GitHub Actions UI without a Claude session.

**Alternatives considered**:

1. **Document-only approach**: Write the schema to CLAUDE.md and rely on Claude to recall it. This is the status quo; it works but costs session tokens and risks schema drift.
2. **Local CLI only**: A bash script without a workflow. Requires cloning the repo locally, which the primary development path (mobile + GitHub web UI) does not support.
3. **Script + workflow (chosen)**: The workflow makes scaffolding accessible from any device with a browser, consistent with the project's no-local-tooling constraint. The script is also runnable locally for developers who have a terminal.

**Consequences**:

- Dispatching the workflow with a slug and title is sufficient to create a buildable (empty) game. The manifest is auto-discovered by `build.yml` on the next push.
- Slug validation (`^[a-z][a-z0-9-]*$`) and directory-existence guard prevent common errors.
- The workflow requires "Read and write permissions" under Settings > Actions > General (already set for `build.yml` and `bundle.yml`).
- New game `.js` files created via the workflow (`scenes/menu.js`) live under `games/<name>/scenes/`, which is below the `maxdepth 1` checked by ADR-0023's registry validation. No registry entry is required for game-specific scenes.
- `scaffold-game.sh` is a `.sh` file, not a `.js` file, so it is not subject to the registry check.

## ADR-0025: `Engine.Balance.diminish` — diminishing-returns softcap primitive

Date: 2026-06-20

**Decision**: Add `Engine.Balance.diminish(x, opts)` to `engine/balance.js`. Returns `x / (x + k)`, mapping `[0, ∞)` to `[0, 1)`. `opts.k` defaults to 100 (the "half-point": the input value that produces a 50% output). Update `docs/resources/balance.md` to promote the softcap row from "deferred" to "implemented" and move the `diminish` description from the Deferred section to the Implemented section.

**Context**: ADR-0020 established `Engine.Balance` with difficulty curves and cost scaling, and documented the deferred roadmap in `balance.md`. Diminishing returns / softcap is the first deferred primitive on the list. The formula `x / (x + k)` is the standard mitigation curve used by World of Warcraft (armor formula before the Wrath-era rework), Path of Exile, and most action RPGs for armor, damage reduction, and general softcaps. Key properties: it maps `[0, ∞)` to `[0, 1)` with no cap configuration needed; it is smooth and monotonically increasing; and effective health `HP / (1 - DR)` is linear in x under this formula, meaning each unit of armor/resistance adds a constant survivability increment regardless of how much the player already has. No finite value of x grants complete immunity (output approaches but never reaches 1), which is mechanically clean.

**Alternatives considered**:

1. **Naming as `reduce`**: Risks shadowing association with `Array.prototype.reduce`. `diminish` is unambiguous in the game-math context.
2. **Hard-capping the output at some maximum (e.g., 75% DR)**: A separate cap can always be applied by the caller (`Math.min(Engine.Balance.diminish(x), 0.75)`). The primitive itself should not embed a game-specific ceiling.
3. **Including the inverse `undim(y) = k*y / (1-y)` (solve for x given DR)**: Useful for authoring ("what armor value gives 40% DR?") but not yet needed by any game. Deferred; trivial to add when needed.

**Consequences**:

- `engine/balance.js` gains a fifth function. Editing it triggers `.github/workflows/bundle.yml`, which regenerates `engine/engine.bundle.js` automatically (ADR-0021). No manual bundle work required.
- The function is pure and stateless, consistent with the `Engine.Balance` namespace contract. It does not call any engine APIs.
- Games using armor or damage-reduction mechanics now have a standard softcap curve to reach for: `const mitigation = Engine.Balance.diminish(armor, { k: 100 })`.
- The `docs/resources/balance.md` deferred list shrinks by one. The next deferred primitive is multiplicative damage: `atk * k / (k + def)`.
- The same formula applies wherever a value should benefit from stacking but plateau gracefully: stat softcaps, healing reduction, shield effectiveness, XP-gain modifiers.
